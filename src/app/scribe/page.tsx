...
export default function ScribePage() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setContent(''); setAuthorName(''); setImageUrl(''); setLinkUrl('');
    ...
    <div>
      <div className="mx-auto max-w-2xl px-4 py-8 pb-24">
        ...
        <div className="text-center my-8">
          ...
          <p className="mt-2 text-lg text-gray-600">Share something with the community.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 bg-white dark:bg-gray-800/50 p-6 rounded-xl border border-gray-200 dark:border-gray-700/50">
          ...
          <div>
            ...
            <Textarea id="content" placeholder="What's on your mind?" value={content} onChange={(e) => setContent(e.target.value)} />
          </div>
          <div>
          ...
          <div>
            ...
            <Input id="linkUrl" type="url" placeholder="https://..." value={linkUrl} onChange={(e) => setLinkUrl(e.target.value)} />
          </div>
          <div>
            <label htmlFor="author" className="block text-sm font-medium mb-1">Your Name (Optional)</label>
            <Input id="author" placeholder="e.g., vitalik.eth" value={authorName} onChange={(e) => setAuthorName(e.target.value)} />
          </div>
          <div>
            <Button type="submit" className="w-full" isLoading={isLoading}>Post Note</Button>
          </div>
        </form>
      </div>
      <Footer />
